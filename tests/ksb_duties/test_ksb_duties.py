...
def test_get_all(client):
    ...
    with patch("app.ksb_duties.models.KsbDuties.select", return_value=mock_query):
        response = client.get("/ksb-duties")
...
def test_get_by_id(ksb_duty_fixture):
    ...
    create_ksb_duty_response = client.post("/ksb-duties", json={
        "ksb_id": ksb_id,
        "duty_id": duty_id
    })
    id = create_ksb_duty_response.get_json()["id"]
    get_response = client.get(f"/ksb-duties/{id}")
    client.delete(f"/ksb-duties/{id}")
...
def test_get_non_existent_record(client):
    response = client.get(f"/ksb-duties/{valid_uuid}")
...
def test_get_record_with_invalid_id(client):
    response = client.get(f"/ksb-duties/{invalid_uuid}")
...
def test_create(client):
    ...
    with patch("app.ksb_duties.models.KsbDuties.create", return_value=mock_model):
        response = client.post("/ksb-duties", json={
            "ksb_id": mock_ksb_id,
...
def test_create_with_non_existent_duty(ksb_duty_fixture):
    ...
    new_record_response = client.post("/ksb-duties", json={
        "ksb_id": ksb_id,
        "duty_id": valid_uuid
    })
...
def test_create_duplicate(ksb_duty_fixture):
    ...
    id = create_ksb_duty_response.get_json()["id"]
    duplicate_response = client.post("/ksb-duties", json={
        "ksb_id": ksb_id,
        "duty_id": duty_id
    })
    client.delete(f"/ksb-duties/{id}")
...
def test_delete(ksb_duty_fixture):
    ...
    id = create_ksb_duty_response.get_json()["id"]
    delete_response = client.delete(f"/ksb-duties/{id}")
...
def test_delete_non_existing_record(client):
    response = client.delete(f"/ksb-duties/{valid_uuid}")
...
def test_delete_record_with_invalid_id(client):
    response = client.delete(f"/ksb-duties/{invalid_uuid}")
...
def test_update(ksb_duty_fixture):
    ...
    id = create_ksb_duty_response.get_json()["id"]
    patch_response = client.patch(f"/ksb-duties/{id}", json={
        "ksb_id": ksb_2_id,
        "duty_id": duty_2_id
    })
    client.delete(f"ksb-duties/{id}")
...
def test_update_non_existing_record(client):
    response = client.patch(f"/ksb-duties/{valid_uuid}", json={
        "ksb_id": valid_uuid,
        "duty_id": valid_uuid,
    })
...
def test_update_record_with_invalid_id(client):
    response = client.patch(f"/ksb-duties/{invalid_uuid}", json={
        "ksb_id": valid_uuid,
        "duty_id": valid_uuid,
    })
